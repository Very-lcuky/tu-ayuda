...
In the browser, the [WebSocket](https://developer.mozilla.org/es-ES/docs/Web/API/WebSocket) object does not support additional headers.
...
- **constructor**
    ...
    - **Websocket-only options**
      - `protocols` (`Array`): a list of subprotocols (see [MDN reference](https://developer.mozilla.org/es-ES/docs/Web/API/WebSockets_API/Writing_WebSocket_servers#Subprotocols))
      - `closeOnBeforeunload` (`Boolean`): whether to silently close the connection when the [`beforeunload`](https://developer.mozilla.org/es-ES/docs/Web/API/Window/beforeunload_event) event is emitted in the browser (defaults to `false`)
- `send`
